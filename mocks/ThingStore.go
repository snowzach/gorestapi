// Code generated by mockery v1.0.0. DO NOT EDIT.

package mocks

import context "context"
import gorestapi "github.com/snowzach/gorestapi/gorestapi"
import mock "github.com/stretchr/testify/mock"

// ThingStore is an autogenerated mock type for the ThingStore type
type ThingStore struct {
	mock.Mock
}

// ThingDeleteByID provides a mock function with given fields: _a0, _a1
func (_m *ThingStore) ThingDeleteByID(_a0 context.Context, _a1 string) error {
	ret := _m.Called(_a0, _a1)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string) error); ok {
		r0 = rf(_a0, _a1)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// ThingFind provides a mock function with given fields: _a0
func (_m *ThingStore) ThingFind(_a0 context.Context) ([]*gorestapi.Thing, error) {
	ret := _m.Called(_a0)

	var r0 []*gorestapi.Thing
	if rf, ok := ret.Get(0).(func(context.Context) []*gorestapi.Thing); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*gorestapi.Thing)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context) error); ok {
		r1 = rf(_a0)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// ThingGetByID provides a mock function with given fields: _a0, _a1
func (_m *ThingStore) ThingGetByID(_a0 context.Context, _a1 string) (*gorestapi.Thing, error) {
	ret := _m.Called(_a0, _a1)

	var r0 *gorestapi.Thing
	if rf, ok := ret.Get(0).(func(context.Context, string) *gorestapi.Thing); ok {
		r0 = rf(_a0, _a1)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*gorestapi.Thing)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, string) error); ok {
		r1 = rf(_a0, _a1)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// ThingSave provides a mock function with given fields: _a0, _a1
func (_m *ThingStore) ThingSave(_a0 context.Context, _a1 *gorestapi.Thing) (string, error) {
	ret := _m.Called(_a0, _a1)

	var r0 string
	if rf, ok := ret.Get(0).(func(context.Context, *gorestapi.Thing) string); ok {
		r0 = rf(_a0, _a1)
	} else {
		r0 = ret.Get(0).(string)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, *gorestapi.Thing) error); ok {
		r1 = rf(_a0, _a1)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
